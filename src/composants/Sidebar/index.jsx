...
function Sidebar({ setLarge, large }) {
   ...
   const showSidebar = () => {
      /*setSidebar(!sidebar);*/ setLarge(!large);
   };
      ...
      <div>
         ...
         >
            <div id="logo" style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
               <div>
                  <img alt="Logo iri" width="50px" height="40px" src={"/images/logoprogrammeleadership.png"} />
               </div>
            ...
            <div style={{ display: "flex", flexDirection: "row", marginRight: "10px", alignItems: "center" }}>
               ...
               >
                  <span>
                     Utilisateur:{" "}
                     <span style={{ margin: "0px", fontSize: "20px", fontWeight: "600" }}> {"mon du user"} </span>
                  </span>
               </div>
               <div
                  onClick={() => {
               ...
               return (
                  <>
                     {5 >= item.userLevel && (
                        ...
                        >
                           <nav
                              style={{ color: indexMenuSelected === item.order ? item.color : "white" }}
                              onClick={() => setSelected(item.order)}
                           ...
                           {indexMenuSelected === item.order ? (
                              item.subMenu ? (
                                 <ScrollMenu
                                    ...
                                    className={""}
                                    mother={item.order}
                                    motherSelected={indexMenuSelected}
                                 />
                              ) : null
                           ) : item.subMenu ? (
                              <ScrollMenu
