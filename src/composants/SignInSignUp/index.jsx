...
export default function SignInSignUp({ signIn, variantButton, classButtom, justTexte }) {
    ...
    return (
        <>
            {justTexte ?
                <span onClick={handleClickOpen} className='justTexte' style={{ cursor: 'pointer' }}>{!signIn ? 'S\'inscrire' : 'Se connecter'} </span>
                :
                <Button className={classButtom ? classButtom : 'boutton01'} variant={variantButton ? variantButton : 'contained'} onClick={handleClickOpen} color='error' >{!signIn ? 'S\'inscrire' : 'Se connecter'}</Button>
            }
            <Dialog
                fullWidth={true}
                maxWidth='md'
    ...
    const handleKeyPress = (event) => {
        if (event.key === 'Enter') {
            if ((activeStep >= steps.length - 1)) {
                setActiveStep(3); setSave(true); setErrorServeur(false)
            } else {
                handleNext()
            }
        }
            ...
            <div className='divFormulaire' style={{ display: 'flex', flexDirection: 'column', }}>
                ...
                {activeStep === 3 &&
                    <fieldset>
                        ...
                        {save &&
                            <ValidationSingInSignUp setErrorServeur={setErrorServeur} formSignUp={formSignUp} setActiveStep={setActiveStep} setError={setError} setSave={setSave} />
                        }
                        {errorServeur &&
                            <MessageErrorServeur />
                        }
                ...
                <div className='divContainButton'>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        <Button variant='outlined' onClick={handleBack} disabled={(activeStep === 0) || (activeStep === 3 && save === true)}>
                            Precedant
                        </Button>
                        {(activeStep >= steps.length - 1) ?
                            <Button variant='contained' onClick={() => { setActiveStep(3); setSave(true); setErrorServeur(false) }} >
                                Enregister
                            </Button>
                            :
...
const SignIn = ({ formSignIn, setFormSignIn, setOpen, setIsSignIn, handleClose, toggleSignInSignUp, }) => {
    ...
    const handleKeyPress = (event) => {
        if (event.key === 'Enter') {
            setSave(true); setErrorServeur(false)
        }
    }
    return (
        <>
            ...
            >
                <div style={{ textAlign: 'center', marginTop: 20, marginBottom: 40 }}>
                    <span className='signInTitle' style={{}} >Connectez Vous...!</span>
                </div>
                ...
                <fieldset >
                    ...
                    {save &&
                        <ValidationSingInSignUp setErrorServeur={setErrorServeur} formSignIn={formSignIn} setSave={setSave} isSignIn={true} setError={setError} />
                    }
                ...
                <div className='divContainButton'>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        <Button variant='contained' onClick={() => { setSave(true); setErrorServeur(false) }} disabled={save}>
                            Connexion
                        </Button>
                    </div>
                ...
                <div style={{ marginTop: 15, cursor: 'pointer' }}>
                    <span>Vous n'avez pas de compte? <span onClick={toggleSignInSignUp} style={{ color: 'green', fontWeight: 'bold' }}>Créer votre compte</span></span>
                </div>
            </div>
        </>
    )
...
const ValidationSingInSignUp = ({ isSignIn, formSignIn, formSignUp, setActiveStep, setError, setSave, setErrorServeur }) => {
