            ...
            <div className="headerPage" style={{ backgroundColor: '#17bd08cc', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }} >
                <div id="Links" style={{ display: "flex", flexDirection: "column", fontWeight: "500", color: 'white' }}>
                    ...
                    <span style={{ fontWeight: "normal", }}>
                        <Link to={'/article'} style={{ textDecorationLine: 'underline', color: 'white' }} >   Arcticles  </Link>
                    </span>
                </div>
                <div>
                    <Button variant='contained' onClick={() => { navigation('/article/creer') }} color='success'>Ajouter</Button>
                                    ...
                                    <Table width={"100%"} hover size="sm" style={{ marginBottom: '0px', }}>
                                        ...
                                        <tbody >
                                            {data.content.map((article, index) => (
                                                <tr key={article.id + "idarticle"} style={{ height: '40px' }}>
                                                    ...
                                                    <td className='body_tr'>{(new Date(article.date)).toLocaleDateString('fr-FR', {
                                                        day: 'numeric',
                                                        month: 'long',
                                                        year: 'numeric',
                                                    })} </td>
                                                    ...
                                                    <td className='body_tr'>
                                                        <div style={{ display: 'flex', flexDirection: 'row', gap: 5 }}>
                                                            ...
                                                            <Button variant='outlined' color='info' sx={{ fontSize: 10, }} onClick={() => { navigation(`/article/${article.id}`) }} >Voir plus</Button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
