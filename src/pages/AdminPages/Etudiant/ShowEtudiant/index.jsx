...
const InfoGeneral = ({ formEtudiant }) => {
    let etudiant = formEtudiant ? formEtudiant.etudiant : {
        matricule: '',
        nom: '',
        prenom: '',
        email: '',
        ...
        <Row style={{ marginTop: 20, marginBottom: 10 }}>
            <Col md={6}>
                <div style={{ backgroundColor: 'white', display: 'flex', flexDirection: 'column', padding: 10, borderRadius: 5 }}>
                    ...
                    <span>Date d'inscription : {(new Date(etudiant.dateInscription)).toLocaleDateString('fr-FR', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                    })}</span>
                    <span>Profession : {etudiant.profession}</span>
                    <span>Région : {etudiant.region}</span>
                </div>
            </Col>
            <Col md={6}>
                <div style={{ backgroundColor: 'white', display: 'flex', flexDirection: 'column', padding: 10, borderRadius: 5 }}>
                    ...
                    <span>QCM Valider : {etudiantChapitres.reduce((total, chapitre) => total + chapitre.qcmValide, 0) + ' sur ' + chapitreSimpleforms.reduce((total, item) => total + item.qcm, 0)} </span>
                    <span>Dernierre Connexion :{(new Date(etudiant.lastConnexion)).toLocaleDateString('fr-FR', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                    })}</span>
        ...
        <Row style={{ marginTop: 20, marginBottom: 10 }}>
            <Col>
                <div style={{ backgroundColor: 'white', display: 'flex', flexDirection: 'column', padding: 10, borderRadius: 5 }}>
                    ...
                    <span>Debuter le :<span>{dateDebut ? (new Date(dateDebut)).toLocaleDateString('fr-FR', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                    }) : '---'}</span></span>

                    <span>Cours commencés :: <span>{(`${courLu} sur ${courTotal}`)}</span></span>
                    <span>QCM Validés: <span>{(`${qcmValider} sur ${totalQcm}`)}</span></span>

                    <span>Statut: <span>{isStarted ? 'Commencer' : 'Non Commencer'}</span></span>
                    {!isStarted &&
                        <div style={{ fontSize: 18, color: appColor.rouge, fontWeight: 700, marginTop: 10, marginBottom: 10, }}>
                                ...
                                <tbody>
                                    {ModuleChapitres.map((chapitre, index) => (
                                        <tr key={chapitre.idChapitre}>
                                            ...
                                            <td>{chapitre.etudiantChapitre ? (new Date(chapitre.etudiantChapitre.dateDebut)).toLocaleDateString('fr-FR', {
                                                day: 'numeric',
                                                month: 'long',
                                                year: 'numeric',
                                            }) : '---'}</td>
                                            ...
                                            <td>{chapitre.qcm}</td>
                                            <td
                                                style={{ fontWeight: 700, color: (chapitre.etudiantChapitre && chapitre.etudiantChapitre.qcmValide >= (chapitre.qcm) / 2) ? 'green' : 'red' }}
                                            >
                                                {((chapitre.etudiantChapitre && (chapitre.etudiantChapitre.qcmValide >= ((chapitre.qcm) / 2)))) ? 'Valider' : 'Non Valider'}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
